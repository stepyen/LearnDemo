 ...
 *
 */
@Route(path = PagePathHub.DemoSoftInputHeightActivity)
class DemoSoftInputHeightActivity : BasePageActivity() {
    override var TAG: String = "DemoSoftInputHeightActivity_TAG"

    override fun initView() {
        addView(R.layout.demo_activity_soft_input_height)
    ...
    private fun softInputHeightListen() {
        ...
        contentView.viewTreeObserver
            .addOnGlobalLayoutListener(ViewTreeObserver.OnGlobalLayoutListener {
                L.d(TAG,"软键盘高度：${getContentViewInvisibleHeight(this@DemoSoftInputHeightActivity)}")
            })
